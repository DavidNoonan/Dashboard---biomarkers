...
simulated_data_population <- tibble(
    ...
    Treatment = as.character(sample(x = c("Treatment 1", "Treatment 2", "Treatment 3"),
                                    size = 200, replace = TRUE)),
    Gender =  sample(x = c("Female", "Male"),
               size = 200, replace = TRUE),
    Age = as.integer(runif(n = 200, min = 18, max = 90)))


# generate 
simulated_data <- replicate(n = 10, expr = simulated_data_population, simplify = FALSE) %>% bind_rows() %>%
    mutate(
    ...
    tabPanel(
        ...
        sidebarLayout(
            sidebarPanel(
                ...
                selectInput(
                    "b1", "Variable 1:",
                    choices <- simulated_data %>% select(-Patient, -Time) %>% colnames(), selected = "Biomarker_1"
                ),
                hr(),
...
server <- function(input, output) {
    ...
    relationship_plot_type <- reactive({
        case_when(
            ...
            all(relationship_variable_classes() == "factor") ~ "bar_plot",
            relationship_variable_classes()[1] == "factor" & relationship_variable_classes()[2] != "factor" ~ "box_plot1",
            relationship_variable_classes()[1] != "factor" & relationship_variable_classes()[2] == "factor" ~ "box_plot2",
            is.na(relationship_variable_classes()[2]) ~ "histogram"
        )
    })
